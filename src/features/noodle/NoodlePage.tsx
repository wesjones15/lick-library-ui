...
export default function NoodlePage() {
  ...
  const { tabHeaderChords, tabOrderMap, tabByHeader } = useMemo(() => {
    const headers = new Set<ChordLyric>();
    const headerChords = new Map<GuitarTabLine, string[]>();
    const orderMap = new Map<GuitarTabLine, number>();
    const byHeader = new Map<ChordLyric, GuitarTabLine>();
    let tabIdx = 0;
    const lines = song?.chordLines ?? [];
    for (let i = 0; i < lines.length; i++) {
      ...
      if (isTabLine(l)) {
        ...
        for (let j = i - 1; j >= 0; j--) {
          const prev = lines[j];
          if (isTabLine(prev)) break;
          const cl = prev as ChordLyric;
          if (cl.chords.trim() || cl.lyrics.trim()) {
            headers.add(cl);
            byHeader.set(cl, l);
            headerChords.set(l, parseChordsFromLine(cl.chords));
            break;
          }
        }
      }
    }
    return { tabHeaderChords: headerChords, tabOrderMap: orderMap, tabByHeader: byHeader };
  }, [song]);

  const contentLines = useMemo((): (ChordLyric | GuitarTabLine)[] => {
    if (!song) return [];
    return song.chordLines.flatMap(l => {
      if (isTabLine(l)) return (lickModeEnabled && !tabHeaderChords.has(l)) ? [l] : [];
      const cl = l as ChordLyric;
      if (!cl.chords.trim() && !cl.lyrics.trim()) return [];
      return [cl];
    });
  }, [song, lickModeEnabled, tabHeaderChords]);
  ...
  useEffect(() => {
    ...
    getBeatmap(activeSongId)
      ...
      .catch(() => {
        const allChordLines = (song?.chordLines ?? []).filter(
          l => !isTabLine(l) && ((l as ChordLyric).chords.trim() || (l as ChordLyric).lyrics.trim())
        ) as ChordLyric[];
        const generated = generateBeatmap(allChordLines, song?.timeSignature ?? 4);
  ...
  const lickOverlayDots = useMemo(() => {
    ...
    if (isTabLine(line)) {
      ...
      // Scenario 1: check if this chord line is a tab header
      const associated = tabByHeader.get(line as ChordLyric);
  ...
  }, [lickModeEnabled, noodleMode, contentLines, currentIdx, intraChordIdx, tabColumnsByTab, tabByHeader, localCapo, instrument]);
