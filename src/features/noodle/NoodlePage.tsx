...
export default function NoodlePage() {
  ...
  const beatmapByLine = useMemo((): number[][] | null => {
    ...
    const expected = contentLines.filter(l => !isTabLine(l)).reduce(
      (n, l) => n + parseChordsFromLine((l as ChordLyric).chords).length, 0
    );
    if (beatmap.length < expected) return null;
    let offset = 0;
  ...
  const karaokeChordIdx = useMemo(() => {
    ...
    const tokens = parseChordsFromLine((contentLines[currentIdx] as ChordLyric).chords);
    const cur = tokens[intraChordIdx];
    if (!cur || cur === 'NC' || cur === 'N.C.') return undefined;
    return tokens.slice(0, intraChordIdx).filter(t => t !== 'NC' && t !== 'N.C.').length;
