...
const BEAT_CYCLE = [0, 1, 2, 4, 8, 16, 3, 6, 5, 7, 9, 10, 11, 12, 13, 14, 15];

function stepBeat(val: number, dir: 1 | -1): number {
  const idx = BEAT_CYCLE.indexOf(val);
  if (idx === -1) return dir === 1 ? BEAT_CYCLE[0] : BEAT_CYCLE[BEAT_CYCLE.length - 1];
  return BEAT_CYCLE[Math.max(0, Math.min(BEAT_CYCLE.length - 1, idx + dir))];
}
...
function generateBeatmap(lines: ChordLyric[]): number[] {
...
export default function NoodlePage() {
  ...
  useEffect(() => {
    ...
    getBeatmap(activeSongId)
      ...
      .catch(() => {
        const generated = generateBeatmap(contentLines);
                        ...
                        {tokens.map((chord, ci) => {
                          ...
                          return (
                            <div key={ci} className="flex flex-col items-center gap-1">
                              ...
                              <div className="flex items-center gap-1">
                                <button onClick={() => set(stepBeat(val, -1))} className="w-7 h-7 flex items-center justify-center text-sm border border-gray-200 rounded text-gray-500 hover:bg-gray-100 active:bg-gray-200">−</button>
            ...
            <div className="flex gap-2 px-5 py-3 border-t border-gray-100 shrink-0">
              <button
                onClick={() => {
                  ...
                  saveBeatmap(activeSongId, beatmapDraft.map(v => v ?? 4))
                    .then(() => {
                ...
                disabled={!song?.ownedByCurrentUser}
                title={song?.ownedByCurrentUser ? undefined : "Only the song owner can save the beatmap"}
                className="px-4 py-1.5 text-xs rounded-lg bg-indigo-600 text-white hover:bg-indigo-700 transition-colors font-medium disabled:opacity-40 disabled:cursor-not-allowed"
              >
                Save
              </button>
              <button
                onClick={() => setShowBeatmapEditor(false)}
