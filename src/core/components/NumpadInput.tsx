...
export default function NumpadInput({
  ...
  const confirmCls = `${btnBase} bg-indigo-600 text-white active:bg-indigo-700`;
  ...
  if (insertMode) {
    return (
      <div
        className="fixed bottom-0 left-0 right-0 bg-white border-t border-gray-200 z-[200] p-3 pb-6"
  ...
  return (
    <>
      ...
      {open && (
        <>
          <div
            className={`fixed inset-0 z-[199] ${wide ? 'bg-transparent' : 'bg-black/30'}`}
            onPointerDown={handleCancel}
          />
          {wide && popPos ? (
            <div
              className="fixed bg-white rounded-xl shadow-xl z-[200] p-3"
              style={{ top: popPos.top, left: popPos.left, width: PAD_W }}
              onPointerDown={e => e.stopPropagation()}
            >
              <div className="text-center text-xl font-bold text-gray-900 tabular-nums mb-3 h-7 flex items-center justify-center">
            ...
            <div
              ...
              onPointerDown={e => e.stopPropagation()}
            >
              <div className="text-center text-4xl font-bold text-gray-900 tabular-nums mb-4 h-12 flex items-center justify-center">
                {draft !== '' ? draft : <span className="text-gray-300">{placeholder ?? '—'}</span>}
              </div>
